...
def scatter(data, centroids):
    ...
    plt.legend()
    #plt.savefig('Clustering_Serial.png')
    plt.show()
...
def heatmap(centroids):
    ...
    #plt.savefig('Heatmap_Serial.png')
    plt.show()
...
if __name__ == '__main__':
    ...
    # Read data
    data_computer = get_data('computers.csv')
    #data_computer = data_computer[:100000, :] # for testing
    print(f'Run with {data_computer.shape[0]} rows started!\n')
    
    # lists for storing results for each k
    cluster_list = []  
    centroids_list = []  
    WCSS_list = []

    # run k-means for k=2 to k_max
    for k in range(1, k_max):
        # Step 1: Initialize centroids
        centroids = initial_centroids_PP(data = data_computer, k = k)  # output: k centroids as arrays
        
        # Step 2: assign all points to a cluster
        initial_clusters = assign_to_centroid(centroids = centroids, data = data_computer)

        # Step 3: calculate new centroids as mean of the clusters
        centroids = get_new_centroids(current_clusters = initial_clusters,
                                      data = data_computer)  # output: k centroids as arrays

        # Step 4: Get final clusters (repeat 3 until no changes in clusters)
        final_clusters, final_centroids = get_final_clusters(
            centroids = centroids, data = data_computer, clustering = initial_clusters, k=k)

        # Step 5: Calculate WCSS for each k
        WCSS = get_WCSS(data=data_computer, clustering = final_clusters, centroids=final_centroids, k=k)
        
        # append lists for storing results
        cluster_list.append(final_clusters)
        centroids_list.append(final_centroids)
    ...
    data_mit_cluster[:, -1] = cluster_list[ideal_k - 1]

    centroids_final = centroids_list[ideal_k - 1]


    # Plot first two dimenasions
    scatter(data=data_mit_cluster, centroids=centroids_final)

    # Calculate average price per cluster
    cluster_labels = np.unique(cluster_list[ideal_k - 1])
