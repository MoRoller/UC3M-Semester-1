...
def run_clustering(d, k, results):
    WCSS, final_clusters, final_centroids = run_all(d, k)  # Call run_all and get the return values
    results[k - 1] = (WCSS, final_clusters, final_centroids)  # Store the results in the list
...
if __name__ == '__main__':
    ...
    threads_list = []
    results = [None] * (n_threads -1)
    
    # number of threads created = number of different k
    for k in range(1, n_threads):
        thread = threading.Thread(target = run_clustering, args = (data_computer, k, results))
